...
void gaussian_blur_omp_loops(int radius, img_t *imgin, img_t *imgout)
{
    ...
    omp_set_dynamic(0);
    #pragma omp parallel for schedule(static) shared(imgin, imgout, width, height, radius) private(i, j, row, col, weightSum, redSum, greenSum, blueSum) num_threads(threads)   
	    for (i = 0; i < height; i++)
	    {
		    for (j = 0; j < width ; j++) 
		    {
...
/* Parallel Gaussian Blur with OpenMP tasks -- every line is a task*/
void gaussian_blur_omp_tasks(int radius, img_t *imgin, img_t *imgout)
{
    ...
    omp_set_dynamic(0);
    //#pragma omp parallel for schedule(static) shared(imgin, imgout, width, height, radius) private(i, j, row, col, weightSum, redSum, greenSum, blueSum) num_threads(threads)
	#pragma omp parallel for schedule(static) shared(height) private(i) num_threads(threads)
		for (i = 0; i < height; i++)
		{
			//#pragma omp task 
			#pragma omp task shared(imgin, imgout, width,radius) private(j, row, col, weightSum, redSum, greenSum, blueSum)
			{
				for (j = 0; j < width ; j++) 
				{
					for (row = i-radius; row <= i + radius; row++)
...
int main(int argc, char *argv[]) 
{
	...
	if (argc < 4)
	{
		fprintf(stderr, "Syntax: %s <blur-radius> <filename> <no of threads>\n",argv[0]);
		exit(1);
	}
